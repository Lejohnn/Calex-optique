                                    ...
                                    <table class="table table-striped table-bordered datatable">
                                        ...
                                        <tbody>
                                            ...
                                            @php
                                                ...
                                                $diffInDays = \Carbon\Carbon::parse($commercial->date_rdv)->diffInDays(now(), false);
                                                    // Détermination de la classe CSS en fonction de la différence en jours
                                                    if ( $diffInDays == -1 OR $diffInDays == -2 OR $diffInDays == -3) {
                                            ...
                                            <tr id="commercial-{{ $commercial->id }}">
                                                ...
                                                <td>
                                                    <button class="btn {{ $buttonClass }}">{{ $commercial->date_rdv }}</button>
                                                </td>
                                                <td>
                                                    <button class="btn {{ $buttonClass }}">{{ $commercial->rdv_heure }}</button>
                                                </td>
                                                ...
                                                @if (auth()->user()->role_id == 1 or auth()->user()->role_id == 8)
                                                    ...
                                                    <td class="{{ $backgroundClass }}">
                                                        <form>
                                                            @csrf
                                                            <select class="form-control validation-status" name="validation_status" data-commercial-id="{{ $commercial->id }}">
                                                                <option class="status-pending" value="pending" {{ $commercial->validation_status == 'pending' ? 'selected' : '' }}>Pending</option>
                                                                <option class="status-confirmed" value="confirmed" {{ $commercial->validation_status == 'confirmed' ? 'selected' : '' }}>Confirmed</option>
                                                                <option class="status-denied" value="denied" {{ $commercial->validation_status == 'denied' ? 'selected' : '' }}>Denied</option>
                                                                <option class="status-peace" value="peace" {{ $commercial->validation_status == 'peace' ? 'selected' : '' }}>Peace</option>
                                                            </select>
                                                        </form>
                                                    </td>
                                                @endif
                                                ...
                                                <td>
                                                    ...
                                                    @if (auth()->user()->role_id == 1 or auth()->user()->role_id == 8)
                                                        ...
                                                        <a href="#" class="edit-rdv-heure-btn" data-id="{{ $commercial->id }}" data-date-rdv="{{ $commercial->date_rdv }}" data-heure-rdv="{{ $commercial->rdv_heure }}" data-toggle="modal" data-target="#editRdvHeureModal">
...
<div class="modal fade" id="editRdvHeureModal" tabindex="-1" role="dialog" aria-labelledby="editRdvHeureModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editRdvHeureModalLabel">Modifier l'heure et la date du Rendez-vous</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="editRdvHeureForm" action="{{ route('commercial.testHeure') }}" method="POST">
                    @csrf
                    @method('PUT')
                    <input type="hidden" id="commercial_id" name="commercial_id">
                    ...
                    <div class="form-group">
                        ...
                        <input type="time" class="form-control" id="rdv_heure" name="rdv_heure" required>
                    </div>
